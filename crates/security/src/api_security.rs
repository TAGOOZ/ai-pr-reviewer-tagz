...
impl SecurityManager {
    ...
    pub async fn check_rate_limit(&self, identifier: &str) -> Result<bool> {
        ...
        if rate_info.request_count > rate_info.burst_size {
            ...
            tracing::warn!(
                "Burst limit exceeded for: {} (count: {})",
                identifier,
                rate_info.request_count
            );
            return Ok(false);
    ...
    fn verify_jwt_signature(&self, token: &str) -> Result<JWTPayload> {
        ...
        mac.verify_slice(&expected_signature).map_err(|_| {
            CodeRabbitError::ConfigError("JWT signature verification failed".to_string())
        })?;
        ...
        let payload_str = String::from_utf8(payload_json).map_err(|e| {
            CodeRabbitError::ConfigError(format!("Payload UTF-8 decode failed: {}", e))
        })?;

        let payload: JWTPayload = serde_json::from_str(&payload_str).map_err(|e| {
            CodeRabbitError::ConfigError(format!("Payload JSON decode failed: {}", e))
        })?;
    ...
    async fn cleanup_expired_tokens(&self) -> Result<()> {
        ...
        let mut active_tokens = self.active_tokens.write().await;
        let expired_tokens: Vec<String> = active_tokens
            .iter()
    ...
    async fn cleanup_rate_limits(&self) -> Result<()> {
        ...
        let mut rate_limits = self.rate_limits.write().await;
        let old_entries: Vec<String> = rate_limits
            .iter()
