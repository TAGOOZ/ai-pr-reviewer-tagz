...
impl ComplianceManager {
    ...
    async fn initialize_default_controls(&self) -> Result<()> {
        ...
        // Update statistics
        {
            ...
            stats.implemented_controls = controls
                .values()
    ...
    fn create_availability_control(
        &self,
        control_id: &str,
        name: &str,
        description: &str,
    ...
    fn create_processing_integrity_control(
        &self,
        control_id: &str,
        name: &str,
        description: &str,
    ...
    fn create_confidentiality_control(
        &self,
        control_id: &str,
        name: &str,
        description: &str,
        ...
        if let Some(control) = controls.get_mut(control_id) {
            ...
            control.next_assessment = match status {
                ControlStatus::Implemented => {
                    Some(chrono::Utc::now() + chrono::Duration::days(180))
                }
    ...
    pub async fn collect_automated_evidence(&self, control_id: &str) -> Result<ControlEvidence> {
        ...
        let evidence = match self.get_control(control_id).await {
            Some(control) => match control.control_type {
                SOC2ControlType::Security => self.collect_security_evidence(control_id).await,
                SOC2ControlType::Availability => {
                    self.collect_availability_evidence(control_id).await
                }
                SOC2ControlType::ProcessingIntegrity => {
                    self.collect_processing_integrity_evidence(control_id).await
                }
                SOC2ControlType::Confidentiality => {
                    self.collect_confidentiality_evidence(control_id).await
                }
                SOC2ControlType::Privacy => self.collect_privacy_evidence(control_id).await,
            },
            None => {
                return Err(CodeRabbitError::NotFound(format!(
                    "Control {} not found",
                    control_id
                )))
            }
        ...
        // Add to control
        self.add_control_evidence(control_id, evidence.clone())
            .await?;
    ...
    async fn collect_availability_evidence(&self, control_id: &str) -> ControlEvidence {
        ...
        ControlEvidence {
            ...
            evidence_type: EvidenceType::MonitoringDashboard,
            description: format!(
                "Automated availability evidence collection for {}",
                control_id
            ),
            file_path: None,
            timestamp: chrono::Utc::now(),
    ...
    async fn collect_processing_integrity_evidence(&self, control_id: &str) -> ControlEvidence {
        ...
        ControlEvidence {
            ...
            description: format!(
                "Automated processing integrity evidence collection for {}",
                control_id
            ),
            file_path: None,
            timestamp: chrono::Utc::now(),
    ...
    async fn collect_confidentiality_evidence(&self, control_id: &str) -> ControlEvidence {
        ...
        ControlEvidence {
            ...
            description: format!(
                "Automated confidentiality evidence collection for {}",
                control_id
            ),
            file_path: None,
            timestamp: chrono::Utc::now(),
    ...
    async fn log_audit_event(
        ...
        if audit_logs.len() > 10000 {
            let len = audit_logs.len();
            audit_logs.drain(0..len - 10000);
        }
    }
    ...
    async fn update_compliance_stats(&self) {
        ...
        if stats.total_controls > 0 {
            stats.compliance_score =
                (stats.implemented_controls as f32 / stats.total_controls as f32) * 100.0;
        }

        // Calculate automated controls percentage
        let automated_controls = controls
            .values()
            .filter(|c| c.evidence.iter().any(|e| e.automated))
            .count();

        if stats.total_controls > 0 {
            stats.automated_controls_percentage =
                (automated_controls as f32 / stats.total_controls as f32) * 100.0;
        }

        tracing::info!(
            "Updated compliance stats: {:.1}% compliant",
            stats.compliance_score
        );
    }
    ...
    pub async fn run_compliance_assessment(&self) -> Result<ComplianceAssessment> {
        ...
        for control in controls {
            ...
            assessment
                .control_scores
                .insert(control.control_id.clone(), control_score);
            ...
            if control_score < 80.0 {
                assessment.findings.push(ComplianceFinding {
                    ...
                    severity: if control_score < 60.0 {
                        "High".to_string()
                    } else {
                        "Medium".to_string()
                    },
                    description: format!(
                        "Control {} has low compliance score: {:.1}%",
                        control.control_id, control_score
                    ),
                    recommendation: format!(
                        "Improve implementation of {} to achieve higher compliance",
    ...
    async fn assess_control(&self, control: &SOC2Control) -> f32 {
        ...
        if !control.evidence.is_empty() {
            let avg_evidence_quality = control
                .evidence
                .iter()
                .map(|e| e.validity_score)
                .sum::<f32>()
                / control.evidence.len() as f32;
    ...
    async fn generate_recommendations(&self, assessment: &ComplianceAssessment) -> Vec<String> {
        ...
        if assessment.overall_score < 80.0 {
            recommendations.push(
                "Overall compliance score is below target. Focus on implementing missing controls."
                    .to_string(),
            );
        }
        ...
        for (control_id, score) in &assessment.control_scores {
            if *score < 60.0 {
                recommendations.push(format!(
                    "Control {} requires immediate attention with score of {:.1}%",
                    control_id, score
                ));
            }
        }
        ...
        let controls = self.get_controls().await;
        let low_automation_controls: Vec<_> = controls
            .iter()
        ...
        if !low_automation_controls.is_empty() {
            recommendations.push(
                "Increase automation of evidence collection to improve efficiency and consistency."
                    .to_string(),
            );
        }
