...
impl RedisOrchestrator {
    ...
    pub async fn enqueue_job(&self, job_type: JobType, payload: String, priority: u8) -> Result<String> {
        ...
        let _: () = con.expire(&metadata_key, 604800)
    ...
    pub async fn process_job(&self, job: Job) -> Result<String> {
        let (job_id, job_type, payload) = match &job {
            Job::ReviewRequest { metadata, request, .. } => {
                (metadata.job_id.clone(), "ReviewRequest", request.clone())
            },
            Job::AnalysisRequest { metadata, request, .. } => {
        ...
        self.update_job_status(&job_id, JobStatus::Processing).await?;
        
        // Simulate processing (replace with actual logic)
        tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
        
        let response = match job {
            Job::ReviewRequest { metadata, request, response_tx } => {
                let result = format!("Review processed successfully for job {}", metadata.job_id);
                let _ = response_tx.send(result.clone());
                result
            },
            Job::AnalysisRequest { metadata, request, response_tx } => {
                let result = format!("Analysis processed successfully for job {}", metadata.job_id);
                let _ = response_tx.send(result.clone());
                result
            }
        };
    ...
    pub async fn get_job_status(&self, job_id: &str) -> Result<JobStatus> {
        ...
        let metadata_key = format!("job:{}", job_id);
        let metadata_json: String = con.hget(&metadata_key, "metadata")
    ...
    async fn update_job_status(&self, job_id: &str, status: JobStatus) -> Result<()> {
        ...
        let _: () = con.hset(&metadata_key, "metadata", serde_json::to_string(&metadata)?)
    ...
    pub async fn get_metrics(&self) -> Result<(u64, u64, u64, u64)> {
            ...
            if let Ok(metadata) = self.get_job_metadata(&job_id).await {
                ...
                match metadata.status {
                    JobStatus::Completed => {
                        ...
                        let processing_time = (metadata.updated_at - metadata.created_at).max(0) as u64;
        ...
        Ok((total_jobs, completed_jobs, failed_jobs, avg_processing_time_ms))
    }
    ...
    pub async fn retry_failed_job(&self, job_id: &str) -> Result<()> {
        ...
        tracing::info!("Retrying job {} (attempt {}/{})", job_id, metadata.retry_count, MAX_RETRIES);
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_job_type_variants() {
        ...
        let review_metadata = JobMetadata::new(
            JobType::ReviewRequest,
            "review".to_string(),
            5
        );
        ...
        let analysis_metadata = JobMetadata::new(
            JobType::AnalysisRequest,
            "analysis".to_string(),
            3
        );
        assert!(matches!(analysis_metadata.job_type, JobType::AnalysisRequest));
    }
