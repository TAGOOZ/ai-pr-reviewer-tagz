...
impl LanguageParser {
    ...
    pub fn parse(&mut self, language: &str, source_code: &str) -> Result<Tree> {
        ...
        self.parser.set_language(language).map_err(|e| {
            CodeRabbitError::AnalysisError(format!("Failed to set language: {}", e))
        })?;

        self.parser.parse(source_code, None).ok_or_else(|| {
            CodeRabbitError::AnalysisError("Failed to parse source code".to_string())
        })
    }
    ...
    fn get_language(&self, language: &str) -> Result<Language> {
        match language.to_lowercase().as_str() {
            ...
            _ => Err(CodeRabbitError::AnalysisError(format!(
