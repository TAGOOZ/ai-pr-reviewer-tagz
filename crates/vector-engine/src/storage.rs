...
impl VectorStorage {
    pub async fn new(table_name: &str) -> Result<Self> {
        tracing::info!("Initializing LanceDB vector storage for table: {}", table_name);
    ...
    pub async fn batch_insert(&self, records: Vec<VectorRecord>) -> Result<()> {
        tracing::info!("Batch inserting {} vector records into LanceDB", records.len());
        ...
        tracing::info!("Batch insert completed. Inserted {} records, {} bytes", 
    ...
    pub async fn update(&self, id: &str, record: VectorRecord) -> Result<()> {
        ...
        if storage_records.contains_key(id) {
            ...
            return Err(CodeRabbitError::Internal(format!("Record {} not found", id)));
        }
    ...
    pub async fn delete(&self, id: &str) -> Result<()> {
        ...
        if let Some(record) = storage_records.remove(id) {
            ...
            
            {
                ...
                stats.storage_size_bytes = stats.storage_size_bytes.saturating_sub(record_size as u64);
            }
            ...
            Err(CodeRabbitError::NotFound(format!("Record {} not found", id)))
        }
    }
    ...
    pub async fn query(&self, embedding: &[f32], limit: usize, filters: Option<HashMap<String, String>>) -> Result<Vec<VectorRecord>> {
        let start_time = std::time::Instant::now();
        
        tracing::info!("LanceDB similarity query: limit={}, filters={:?}", limit, filters);
        ...
        let results: Vec<VectorRecord> = results.into_iter()
        ...
        tracing::info!("Query completed: {} results in {:.0}ms", results.len(), start_time.elapsed().as_millis() as f64);
        
        Ok(results)
    }

    /// RAG-specific similarity search with context enrichment
    pub async fn rag_search(&self, query: &str, embedding: &[f32], limit: usize, context_filter: Option<String>) -> Result<Vec<RAGSearchResult>> {
        tracing::info!("RAG search for query: '{}' with context: {:?}", query, context_filter);
        ...
        rag_results.sort_by(|a, b| b.relevance_score.partial_cmp(&a.relevance_score).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub async fn hybrid_search(&self, query: &str, embedding: &[f32], limit: usize, keywords: Vec<String>) -> Result<Vec<HybridSearchResult>> {
    ...
    pub async fn create_index(&self, column: &str, index_type: &str) -> Result<()> {
        tracing::info!("Creating LanceDB {} index on column: {}", index_type, column);
    ...
    pub async fn optimize(&self) -> Result<()> {
        ...
        tracing::info!("Optimization completed in {:.0}ms", start_time.elapsed().as_millis() as f64);
        Ok(())
    }
    ...
    fn passes_context_filter(&self, record: &VectorRecord, context: &str) -> bool {
        ...
        record.content.to_lowercase().contains(&context.to_lowercase())
    }
}
