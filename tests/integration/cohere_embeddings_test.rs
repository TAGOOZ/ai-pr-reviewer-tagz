...
async fn test_cohere_generate_embeddings() {
    ...
    println!("🧠 Generating embeddings for {} texts using Cohere", texts.len());
...
"#.to_string(),
    ...
    println!("🧠 Generating code embeddings for {} snippets", code_snippets.len());

    match cohere.generate_code_embeddings(code_snippets.clone()).await {
        Ok(embeddings) => {
            println!("✅ Successfully generated {} code embeddings", embeddings.len());
...
async fn test_cohere_query_embeddings() {
    ...
    println!("🧠 Generating query embeddings for {} queries", queries.len());

    match cohere.generate_query_embeddings(queries.clone()).await {
        Ok(embeddings) => {
            println!("✅ Successfully generated {} query embeddings", embeddings.len());
            assert_eq!(embeddings.len(), 2);
...
async fn test_cohere_chunked_embeddings() {
    ...
    println!("🧠 Generating chunked embeddings for {} texts (chunk size: 3)", texts.len());

    match cohere.generate_embeddings_chunked(texts.clone(), 3).await {
        Ok(embeddings) => {
            println!("✅ Successfully generated {} embeddings in chunks", embeddings.len());
...
async fn test_cohere_different_models() {
    ...
    let cohere = CohereEmbeddings::new(api_key)
        .with_model("embed-multilingual-v3.0".to_string());
