...
#[cfg(test)]
mod tests {
    use coderabbit_api_gateway::helpers::git_client::{
        AzureDevOpsClient, GitHubClient, GitLabClient,
    };
    ...
    async fn real_gitlab_fetch_mr_files() {
        ...
        println!(
            "🔍 Fetching files from GitLab project {} MR !{}",
            project_id, mr_iid
        );
    ...
    async fn real_github_language_detection() {
        ...
        if let Ok(files) = result {
            ...
            let non_plaintext: Vec<_> =
                files.iter().filter(|f| f.language != "plaintext").collect();
    ...
    async fn real_azure_devops_fetch_pr() {
        ...
        let repo_id =
            std::env::var("TEST_AZURE_REPO").expect("Set TEST_AZURE_REPO for Azure DevOps testing");
