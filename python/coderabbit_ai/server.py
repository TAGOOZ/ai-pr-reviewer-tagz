...
async def bridge_analysis_file_batch(payload: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Analysis completed: {len(file_changes)} files analyzed, {len(review_response.comments)} comments generated")
        ...
        raise HTTPException(status_code=500, detail=f"bridge_analysis_file_batch failed: {str(e)}")
...
async def bridge_embedding_batch(payload: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"bridge_embedding_batch failed: {str(e)}")


@app.post("/review/async")
async def process_review_async(request: ReviewRequest, background_tasks: BackgroundTasks):
