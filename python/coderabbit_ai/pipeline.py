...
class CodeRabbitMultiAgentPipeline(dspy.Module):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if use_codeact:
            ...
            self.req_validator_codeact = RequirementsValidatorCodeAct(
                sandbox, max_retries=max_retries
            )
            self.business_logic_codeact = BusinessLogicAnalyzerCodeAct(
                sandbox, max_retries=max_retries
            )
            self.metrics_codeact = MetricsGeneratorCodeAct(
                sandbox, max_retries=max_retries
            )
        else:
    ...
    def forward(self, request: ReviewRequest) -> ReviewResponse:
        ...
        self.line_mappings = self._parse_line_numbers_from_diffs(
        ...
        review_response = self.review_agent.forward(
            context_response, code_changes, org_config
        )

        # Step 3: Verification Agents (Parallel)
        pr_description = (
            request.pull_request.description if hasattr(request, "pull_request") else ""
        )
        ...
        if self.req_validator_codeact:
            codeact_results = self._run_codeact_analysis(
                request, context_response, code_changes, review_response
            )
        ...
        final_comments = self._generate_final_comments(
            review_response, verification_responses, consensus, codeact_results
        )
        ...
        try:
            ...
            if (
                hasattr(context_data, "security_findings")
                and context_data.security_findings
            ):
                security_findings_list.extend(context_data.security_findings)
                logger.info(
                    f"Phase 3: Found {len(context_data.security_findings)} AST-Grep findings"
                )
            ...
            comment_security_findings = self._comments_to_security_findings(comments)
            security_findings_list.extend(comment_security_findings)
            logger.info(
                f"Phase 3: Converted {len(comment_security_findings)} comments to security findings"
            )

            # Run Security Aggregator
            context_metadata = getattr(context_response, "metadata", {})
            prioritized_findings, security_summary, security_recommendation = (
                self.security_aggregator.aggregate(
            ...
            logger.info(
                f"Phase 3: Aggregated {len(prioritized_findings)} security findings, "
        ...
        if config.get_env_bool("ENABLE_PR_TEST_RUNNER", False):
            try:
                ...
                test_result = pr_test_runner.run_tests(
                    ...
                    test_command=getattr(request.config, "test_command", None)
                    if hasattr(request, "config")
                    else None,
                )

                logger.info(
                    f"Phase 4: Tests {'passed' if test_result.passed else 'FAILED'} "
                ...
                if not test_result.passed:
                    ...
                    test_comment_id = hashlib.md5(
                        f"test_failure_{test_result.test_command}".encode()
                    ).hexdigest()[:8]
        ...
        if hasattr(context_response, "metadata"):
            rag_metrics = {
                ...
                "rag_patterns_found": context_response.metadata.get(
                    "rag_patterns_found", 0
                ),
                "rag_issues_found": context_response.metadata.get(
                    "rag_issues_found", 0
                ),
                "rag_practices_found": context_response.metadata.get(
                    "rag_practices_found", 0
                ),
            }
    ...
    def _prepare_context_data(self, request: ReviewRequest) -> ContextData:
        ...
        static_analysis_results, security_findings = self._run_static_analysis(
            request.pull_request.files_changed, project_root=project_root
        )
    ...
    def _gather_historical_data(self, request: ReviewRequest) -> str:
        ...
        try:
            ...
            for file_change in request.pull_request.files_changed[
                :5
                ...
                try:
                    ...
                    result = subprocess.run(
                        ...
                        text=True,
                        timeout=5,
                        cwd=os.path.dirname(file_change.path)
    ...
    def _run_static_analysis(
        self, files_changed: List[Any], project_root: str = "."
        ...
        try:
            ...
            # Extract file paths
            changed_file_paths = [
                fc.path for fc in files_changed if hasattr(fc, "path")
            ]
            ...
            aggregator = StaticAnalysisAggregator(
                enable_astgrep=True, enable_linters=True
            )
            ...
            logger.info(
                f"Static analysis complete: {len(linter_results)} linter results, "
            ...
            logger.warning(
                f"StaticAnalysisAggregator not available: {e}, skipping static analysis"
            )
            return ([], [])
        except Exception as e:
        ...
        try:
            ...
            if requirements_text and self.req_validator_codeact:
                pr_description = (
                    ...
                    if hasattr(request, "pull_request")
                    else ""
                )
                ...
                if req_result.get("success"):
                    ...
                    logger.warning(
                        f"Requirements validation failed: {req_result.get('error')}"
                    )
        except Exception as e:
            ...
            if complexity_score > 0.6 or self._has_async_code(code_changes):
                if self.business_logic_codeact:
                    ...
                    if bl_result.get("success"):
                        ...
                        logger.warning(
                            f"Business logic analysis failed: {bl_result.get('error')}"
                        )
        except Exception as e:
        ...
        try:
            if self.metrics_codeact:
                ...
                if metrics_result.get("success"):
                    ...
                    logger.info(
                        f"Custom metrics: {metrics_result.get('summary', 'N/A')}"
                    )
                else:
                    logger.warning(
                        f"Metrics generation failed: {metrics_result.get('error')}"
                    )
        except Exception as e:
    ...
    def _generate_final_comments(
        ...
        if (
            hasattr(review_response, "review_findings")
            and review_response.review_findings
        ):
        ...
        if consensus.get("consensus_quality") in ["high", "medium"]:
            ...
            if verified_findings:
                ...
                verification_comments = self._parse_verification_findings(
                    verified_findings, consensus
                )
    ...
    def _parse_review_findings(self, findings_text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if line.startswith("File:"):
                ...
                try:
                    current_finding["line_number"] = int(
    ...
    def _parse_verification_findings(
        self, findings_text: str, consensus: Dict[str, Any]
    ) -> List[ReviewComment]:
        ...
        for section in sections[1:]:  # Skip first empty section
            ...
            for idx, item in enumerate(items):
                ...
                item_confidence = min(
                    1.0, base_consensus_score * confidence_variation * position_factor
                )
    ...
    def _parse_codeact_findings(
        self, codeact_results: Dict[str, Any]
    ) -> List[ReviewComment]:
        ...
        if "requirements" in codeact_results:
            ...
            if status == "INCOMPLETE":
                ...
                if missing:
                    ...
                    message += (
                        f"Implemented {implemented_count}/{required_count} features\n"
                    )
        ...
        if "business_logic" in codeact_results:
            ...
            if race_conditions:
                ...
                message += (
                    f"Found {len(race_conditions)} potential race condition(s):\n"
                )
    ...
    def _estimate_ai_cost(
        self, request: ReviewRequest, verification_responses: List[Any]
    ) -> float:
        """Estimate the AI API cost for this review."""
        # Simple cost estimation based on content size
        total_chars = sum(
            len(fc.content) + len(fc.diff) for fc in request.pull_request.files_changed
        )
    ...
    def _comments_to_security_findings(
        self, comments: List[ReviewComment]
        ...
        for comment in comments:
            ...
            if any(
                kw in message_lower
                for kw in ["sql injection", "sql query", "database query"]
            ):
                category = "injection"
                rule_id = "sql-injection-detected"
            elif any(
                kw in message_lower
                for kw in ["xss", "cross-site scripting", "unsafe html"]
            ):
                category = "xss"
                rule_id = "xss-vulnerability-detected"
            elif any(
                kw in message_lower
                for kw in ["hardcoded", "credential", "secret", "password", "api key"]
            ):
                category = "secrets"
                rule_id = "hardcoded-credentials"
            elif any(
                kw in message_lower
                for kw in ["authentication", "authorization", "session"]
            ):
                category = "authentication"
                rule_id = "auth-vulnerability"
            elif any(
                kw in message_lower for kw in ["input validation", "sanitiz", "escap"]
            ):
                category = "input-validation"
                rule_id = "missing-input-validation"
            elif any(
                kw in message_lower
                for kw in ["crypto", "encryption", "hash", "md5", "sha1"]
            ):
    ...
    def _parse_line_numbers_from_diffs(
        self, file_changes: List
...
class PipelineOptimizer:
    ...
    def optimize(
        ...
        # Run optimization
        print(
            f"Starting MIPRO optimization with {len(train_examples)} training examples..."
        )
        optimized_pipeline = optimizer.compile(
            self.pipeline,
            trainset=train_examples,
            valset=val_examples[
                : min(50, len(val_examples))
    ...
    def evaluate(self, test_data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for item in test_data:
            try:
                ...
                fp = len(
                    (predicted_issues & invalid_issues)
                    | (predicted_issues - expected_issues - invalid_issues)
                )
                ...
                if hasattr(prediction, "metrics") and hasattr(
        ...
        for comment in comments:
            ...
            if any(
                kw in message_lower
                for kw in ["sql injection", "sql query", "execute query"]
            ):
                category = "injection"
                rule_id = "sql-injection-detected"
            elif any(
                kw in message_lower
                for kw in ["xss", "cross-site scripting", "render_template_string"]
            ):
                category = "xss"
                rule_id = "xss-vulnerability-detected"
            elif any(
                ...
                kw in message_lower
                for kw in ["md5", "sha1", "weak hash", "weak crypto"]
            ):
                category = "crypto"
                ...
                kw in message_lower
                for kw in ["syntax error", "will not execute", "will break"]
            ):
                category = "syntax"
                rule_id = "critical-syntax-error"
            elif any(
                kw in message_lower
                for kw in ["security vulnerabilit", "security issue"]
            ):
                category = "security"
